...
Last updated 2026-08-03. Last change: **v2's data mix is built and verified and the run is staged, not launched.** 1,228 train rows, 2,456 iters, `valid`/`test` proven byte-identical to v1's. The model server is deployed and serving on the OCI node, closing P2. **Two things are queued behind the v2 launch and both live entirely on the node**: the 96-case local-vs-node score comparison, and P3's daily eval DAG (size its timeouts against 2.5 h, not the stale 40-60 min).
...
- **Comparison-avoidance risk did NOT materialise, on one suite.** The teacher refused 36.0% of comparison cases and the concern was the student inheriting it. On the golden 96 the student refused **24 of 24 adversarial and 0 of 24 comparison**. One suite is not the distribution, but the feared direction is absent; comparison's 0.856 is the lowest category for reasons other than refusal
...
### Thread A — v2 is staged and awaiting launch
